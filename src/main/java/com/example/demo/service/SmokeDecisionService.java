...
public class SmokeDecisionService {
     ...
     * 返回云端智能判定结果。
     */
    public synchronized SmokeDecision analyze(double smokeValue) {
        ...
        if (cloudBaseline == null) {
            ...
            cloudBaseline = Math.max(smokeValue, 1.0);

            addToWindow(smokeValue);
         ...
         * ========================================
         */
        if (smokeWindow.size() < WINDOW_SIZE) {
    ...
    private void addToWindow(double value) {
        ...
        smokeWindow.addLast(value);

        if (smokeWindow.size() > WINDOW_SIZE) {
            smokeWindow.removeFirst();
        }
    }
